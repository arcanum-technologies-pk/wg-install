...
new_client_dns () {
    ...
    # Optionally, you can output the selected DNS servers
...
[Interface]
Address = 10.7.0.1/8$([[ -n "$ip6" ]] && echo ", fddd:2c4:2c4:2c4::1/64")
...
EOF
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd --zone=trusted --add-source=10.7.0.0/8
		firewall-cmd --permanent --add-port="$port"/udp
		firewall-cmd --permanent --zone=trusted --add-source=10.7.0.0/8
		# Set NAT for the VPN subnet
		firewall-cmd --direct --add-rule ipv4 nat POSTROUTING 0 -s 10.7.0.0/8 ! -d 10.7.0.0/8 -j SNAT --to "$ip"
		firewall-cmd --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 10.7.0.0/8 ! -d 10.7.0.0/8 -j SNAT --to "$ip"
		if [[ -n "$ip6" ]]; then
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 10.7.0.0/8 ! -d 10.7.0.0/8 -j SNAT --to $ip
ExecStart=$iptables_path -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -I FORWARD -s 10.7.0.0/8 -j ACCEPT
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 10.7.0.0/8 ! -d 10.7.0.0/8 -j SNAT --to $ip
ExecStop=$iptables_path -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 10.7.0.0/8 -j ACCEPT
...
EOF
    ...
    # Automatically proceed to add a new client
    option="1"
	case "$option" in
		1)
			echo
    ...
    client=$(date +"%Y%m%d_%H%M%S")
		...
		3)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s 10.7.0.0/8 '"'"'!'"'"' -d 10.7.0.0/8' | grep -oE '[^ ]+$')
					# Using both permanent and not permanent rules to avoid a firewalld reload.
					firewall-cmd --remove-port="$port"/udp
					firewall-cmd --zone=trusted --remove-source=10.7.0.0/8
					firewall-cmd --permanent --remove-port="$port"/udp
					firewall-cmd --permanent --zone=trusted --remove-source=10.7.0.0/8
					firewall-cmd --direct --remove-rule ipv4 nat POSTROUTING 0 -s 10.7.0.0/8 ! -d 10.7.0.0/8 -j SNAT --to "$ip"
					firewall-cmd --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 10.7.0.0/8 ! -d 10.7.0.0/8 -j SNAT --to "$ip"
