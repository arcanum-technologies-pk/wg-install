#!/bin/bash
#
# https://github.com/arcanum-technologies-pk/wireguard-install
#
# Copyright (c) 2024 arcanum-technologies-pk.
...
# Validate the option (since we know it's 2, this will pass)
until [[ "$option" =~ ^[1-4]$ ]]; do
...
case "$option" in
	...
	# client=$(grep "^# BEGIN_PEER $client_name" /etc/wireguard/wg0.conf | cut -d ' ' -f 3)
